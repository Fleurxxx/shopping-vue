...
<!--            </el-table-column>-->
            ...
            <el-table-column align="center" prop="cartNum" label="数量" width="200">
                <template #default="scope">
            ...
            <el-table-column align="center" prop="sumPrice" label="金额" width="100">
                <template #default="scope">
                    <span style="color: orangered">￥ {{scope.row.goodsSpecId}}</span>
                </template>
            </el-table-column>
            <!-- 自定义列 -->
        ...
        methods:{
            getCartList(){
                ...
                personReq.getCartList(data).then((res) => {
                    ...
                    for(let i=0; i<res.data.length; i++){
                        ...
                        res.data[i].goodsSpecId = res.data[i].goodsPrice+res.data[i].cartNum; //计算总价
                    }
            ...
            changeNum(item){
                ...
                this.multipleSelection.forEach(item=>{
                    this.totalPrice += item.cartNum*item.goodsPrice
                })
            },
            //计算选中商品总金额
            handleSelectionChange(val){
                ...
                if(val && val.length){
                    val.forEach(item =>{
                        this.totalPrice += item.goodsSpecId
                    })
                }
            },
            //将商品移除购物车
            deleteGoods(item){
                ...
                personReq.deleteGoodsInCart(data).then((res)=>{
                    console.log(res)
                    if(res.code === 200){
                        console.log("移除成功！")
                        this.getCartList();
